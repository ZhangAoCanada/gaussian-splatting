...
from gaussian_renderer import render, network_gui, prefilter_voxel, render_param
import sys
from scene.gaussian_model3.init3 import Scene, GaussianModel
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, enable_tmp=True):
    ...
    gs_dataloader = CacheDataLoader(gs_dataset, max_cache_num=8, seed=42, shuffle=True, num_workers=8, batch_size=1)
    ...
    while iteration <= opt.iterations:
        for dataset_index, viewpoint_cam in enumerate(gs_dataloader):
            ...
            ############# NOTE: for training the model ############
            # visibility_mask = prefilter_voxel(viewpoint_cam, gaussians, pipe, bg)
            # gaussians_visible = gaussians.gs_param(visibility_mask, replace=replace)
            # gaussians_visible = gaussians.gs_allparam(replace=False)
            gaussians_visible = gaussians.gs_numparam(replace=False)
            ### NOTE: normalize gaussians
            forward_kwargs = {
                "batch_normalized_gs": [gaussians.normalize_params(gaussians_visible)],
                "batch_scene_idx": [0]
            }
            with torch.cuda.amp.autocast(enabled=enable_tmp):
                gaussians_learned = model(**forward_kwargs)
            ### NOTE: unormalize gaussians
            gaussians_learned = gaussians.unnormalize_params(gaussians_learned[0])
            #######################################################

            # render_pkg = render(viewpoint_cam, gaussians, pipe, bg)
            render_pkg = render_param(gaussians.act_covariance, viewpoint_cam, gaussians.act_param(gaussians_learned[0]), pipe, bg)
            ...
            ##### TODO: detach after training #####
            # gaussians.update_param(gaussians_learned)
            # gaussians.update_allparam(gaussians_learned)
            # print("[INFO] num gaussians: ", gaussians.get_xyz.shape[0])
            ...
            with torch.no_grad():
                ...
                # Log and save
                training_report(tb_writer, gs_testloader, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, background))
...
def training_report(tb_writer, test_loader, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs):
    if tb_writer:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[i*100 for i in range(1, 51)])
