...
class GaussianModel:
    ...
    def add_raypoints(self, cam, pred_image, gt_image, ssim_threshold=0.5):
        ...
        # self._xyz = torch.cat((xyz_add, self._xyz), dim=0)
        # self._features_dc = torch.cat((features_dc_add, self._features_dc), dim=0)
        # self._features_rest = torch.cat((features_rest_add, self._features_rest), dim=0)
        # self._scaling = torch.cat((scaling_add, self._scaling), dim=0)
        # self._rotation = torch.cat((rotation_add, self._rotation), dim=0)
        # self._opacity = torch.cat((opacity_add, self._opacity), dim=0)
        # self.max_radii2D = torch.cat((max_radii2D_add, self.max_radii2D), dim=0)
        self.len_ray_pts = len_ray_pts_add + self.len_ray_pts

    def prune_ray_points(self, training_args, start_training=False):
    ...
    def create_from_random(self, cam, spatial_lr_scale : float):
        ...
        # rot_mat = rotation_align_vectors_batch(rays_d, torch.tensor([0, 0, 1], device="cuda").float().repeat(rays_d.shape[0], 1)).inverse()
        rot_mat = rotation_align_vectors_batch(torch.tensor([0, 0, 1], device="cuda").float().repeat(rays_d.shape[0], 1), rays_d)
        rots = rotation2quaternion(rot_mat)

        points = rays_o + rays_d * ((self.far - self.near) / 2 + self.near)
        # points = rays_o + rays_d * (((torch.rand((rays_o.shape[0], 1) , device="cuda") - 0.5) * 0.1 + 0.5) * (self.far - self.near) + self.near)
        # scales = torch.ones((points.shape[0], 3), device="cuda") * 0.01
        dist2 = torch.clamp_min(distCUDA2(torch.from_numpy(np.asarray(points.cpu())).float().cuda()), 0.0000001)
        scales = torch.log(torch.sqrt(dist2))[...,None].repeat(1, 3)
        scales[:, 2] = self.scaling_inverse_activation(torch.ones((points.shape[0]), device="cuda") * (self.far - self.near) / 2) # + torch.randn((points.shape[0]), device="cuda") * 0.1)
        # scales[:, 2] = 5
        opacities = inverse_sigmoid(0.1 * torch.ones((points.shape[0], 1), dtype=torch.float, device="cuda"))

        image = cam.original_image.cuda()
        colors = torch.nn.functional.interpolate(image.unsqueeze(0), size=(H, W), mode='bilinear', align_corners=False).squeeze(0).permute(1, 2, 0)
        colors = colors.view(-1, 3).float()
        ...
        # self._rays_o = rays_o
        # self._rays_d = rays_d
        # self._xyz = nn.Parameter(inverse_sigmoid(torch.rand((points.shape[0], 1), device="cuda").float()).requires_grad_(True))
    ...
    def create_from_random2(self, cam, spatial_lr_scale : float):
        self.spatial_lr_scale = spatial_lr_scale
        w2c = cam.world_view_transform.transpose(1, 0)
        c2w = torch.inverse(w2c)
    ...
    def visualize_pnts(self, cam):
        points = self.get_xyz[self.len_ray_pts:].detach()
        opacities = self.get_opacity[self.len_ray_pts:].detach()
