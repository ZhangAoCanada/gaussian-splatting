...
class GaussianModel:
    ...
    def get_xyz(self):
        return self._xyz
        # return self._rays_o + self._rays_d * (torch.sigmoid(self._xyz) * (100-0.1) + 0.1)
    ...
    #     points = R.matmul(points.detach().unsqueeze(-1)).squeeze(-1) + T.unsqueeze(0)
    ...
    ##########################################################################
    ##########################################################################
    def create_coeff(self, spatial_lr_scale : float):
        self.spatial_lr_scale = spatial_lr_scale
    ...
    def create_from_random(self, cam, spatial_lr_scale : float):
        ...
        # fused_point_cloud = torch.rand((1000000, 3), device="cuda").float().cuda()
        # fused_point_cloud = (fused_point_cloud - 0.5) * 2 * 100.
        # fused_color = torch.rand((fused_point_cloud.shape[0], 3), device="cuda").float().cuda()

        # x_range = [-50., 50., 1]
        # y_range = [-50., 50., 1]
        # z_range = [0., 100., 1]
        # x = torch.arange(x_range[0], x_range[1], x_range[2]).cuda()
        # y = torch.arange(y_range[0], y_range[1], y_range[2]).cuda()
        # z = torch.arange(z_range[0], z_range[1], z_range[2]).cuda()
        # xx, yy, zz = torch.meshgrid(x, y, z)
        # fused_point_cloud = torch.stack((xx, yy, zz), dim=-1)
        # fused_point_cloud = fused_point_cloud.reshape(-1, 3).float().cuda()
        # R = c2w[:3, :3]
        # T = c2w[:3, 3]
        # fused_point_cloud = R.matmul(fused_point_cloud.t()).t() + T
        # fused_color = torch.rand((fused_point_cloud.shape[0], 3), device="cuda").float().cuda()
        ...
        ######################## NOTE: store for later ####
        self.points_loc = fused_point_cloud.float().clone()
        self.colors_loc = fused_color.float().clone()
        ###################################################
        ...
        scales = torch.log(torch.sqrt(dist2))[...,None].repeat(1, 3)
        # scales = torch.log(torch.ones((fused_point_cloud.shape[0], 3), device="cuda")*0.1)
        rots = torch.zeros((fused_point_cloud.shape[0], 4), device="cuda")
        rots[:, 0] = 1
    ...
    def create_from_raypoints(self, cam, spatial_lr_scale : float):
        self.spatial_lr_scale = spatial_lr_scale
        w2c = cam.world_view_transform.transpose(1, 0)
        c2w = torch.inverse(w2c)
        ...
        self.far = 100.0
        N_sample = 1000
        self.W, self.H = cam.image_width // self.downsample_ratio, cam.image_height // self.downsample_ratio
        fov_x, fov_y = cam.FoVx, cam.FoVy
        focal_x = fov2focal(fov_x, self.W)
        focal_y = fov2focal(fov_y, self.H)
        rays_o, rays_d = get_rays_wh(self.H, self.W, focal_y, focal_x, c2w)

        # z_val = torch.linspace(near, far, N_sample).cuda()
        # fused_point_cloud = rays_o[..., None, :] + rays_d[..., None, :] * z_val[..., :, None]
        # image = cam.original_image.cuda()
        # fused_color = torch.nn.functional.interpolate(image.unsqueeze(0), size=(H, W), mode='bilinear', align_corners=False).squeeze(0).permute(1, 2, 0)
        # fused_color = fused_color[:, :, None, :].repeat(1, 1, N_sample, 1)
        # fused_point_cloud = fused_point_cloud.float().reshape(-1, 3)
        # fused_color = fused_color.float().reshape(-1, 3)

        z_val = torch.sigmoid(torch.randn((self.H, self.W, 1), device="cuda")) * (self.far - self.near) + self.near
        fused_point_cloud = rays_o + rays_d * z_val
        image = cam.original_image.cuda()
        ...
        self._rays_o = rays_o
        self._rays_d = rays_d
        self._xyz = nn.Parameter(torch.rand((fused_point_cloud.shape[0], 1), device="cuda").requires_grad_(True))
        # self._xyz = nn.Parameter(fused_point_cloud.requires_grad_(True))
        self._features_dc = nn.Parameter(features[:,:,0:1].transpose(1, 2).contiguous().requires_grad_(True))
        self._features_rest = nn.Parameter(features[:,:,1:].transpose(1, 2).contiguous().requires_grad_(True))
        # self._scaling = nn.Parameter(0.01 * torch.randn((fused_point_cloud.shape[0], 3), dtype=torch.float, device="cuda").requires_grad_(True))
        self._scaling = nn.Parameter(scales.requires_grad_(True))
        self._rotation = nn.Parameter(rots.requires_grad_(True))
        self._opacity = nn.Parameter(opacities.requires_grad_(True))
        # self._opacity = nn.Parameter(torch.randn((fused_point_cloud.shape[0], 1), dtype=torch.float, device="cuda").requires_grad_(True))
        self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device="cuda")


    def prune_ray(self, cam):
        ...
        ### NOTE: points_view with x, y and choose the minimum z val
        # sorts = torch.argsort(points[:, 2])
        ...
        ranks = points_2d[:, 0] // 2 * H + points_2d[:, 1] // 2
        ...
        mask3 = torch.logical_and(points_view[:, 2] < self.far, points_view[:, 2] > self.near)
        ...
        plot_points(points_view, colors)


        # depth_map = torch.zeros((H, W), device="cuda")
        # depth_map2 = torch.zeros((H, W), device="cuda")
        # depth_map2_opacities = torch.zeros((H, W), device="cuda")
        # new_points = []
        # new_colors = []
        # for idx, point in enumerate(points_2d):
        #     if points_view[idx, 2] >= depth_map[point[1], point[0]] and depth_map[point[1], point[0]] != 0:
        #         continue
        #     else:
        #         new_points.append(points_view[idx])
        #         new_colors.append(colors[idx])
        #         depth_map[point[1], point[0]] = points_view[idx, 2]
        #     if opacities[idx] <= depth_map2_opacities[point[1], point[0]]:
        #         continue
        #     else:
        #         depth_map2_opacities[point[1], point[0]] = opacities[idx]
        #         depth_map2[point[1], point[0]] = points_view[idx, 2]
        # new_points = torch.stack(new_points)
        # new_colors = torch.stack(new_colors)
        # plot_points(new_points[new_points[:, 2] < 20], new_colors[new_points[:, 2] < 20])
